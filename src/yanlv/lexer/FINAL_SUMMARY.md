...
原始的lexer.py文件过于庞大（570KB，14076行，391个函数），已成功拆分为10个专注的模块：
...
#### 3.1 词元定义 (lexer_token.py)
- TokenType枚举：定义59种词元类型
- Token类：词元数据结构
- 工具函数：创建各种类型的词元

#### 3.2 分词器 (tokenizer.py)
...
#### 3.3 词元匹配器 (matcher.py)
- ITokenMatcher接口：匹配器抽象接口
- TokenMatcher：基础匹配器实现
- AdvancedTokenMatcher：高级匹配器
- 支持自定义词元类型和验证函数

#### 3.4 模式管理器 (pattern_manager.py)
...
#### 3.5 错误处理 (error_handler.py)
- ErrorHandler：错误处理器
- LexerException：词法分析异常基类
- 各种具体异常类
- 支持错误统计和格式化输出

#### 3.6 上下文管理 (context_manager.py)
- ContextManager：上下文管理器
- Context：上下文信息
- 支持上下文栈、符号表和作用域管理

#### 3.7 性能优化器 (performance_optimizer.py)
...
```python
# 使用不同的分词器
lexer = create_lexer("thulac", verbose=True)

# 配置优化级别
from yanlv.lexer import PerformanceOptimizer, OptimizationLevel
...
# 自定义模式
from yanlv.lexer import PatternManager, TokenType
